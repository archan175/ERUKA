      ...
      <section className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border pb-8">
        <div className="flex items-center gap-5">
          <div className="relative group cursor-pointer" onClick={() => setProfileOpen(true)}>
            {avatarData ? (
              <img src={avatarData} alt="avatar" className="h-20 w-20 rounded-2xl object-cover border-2 border-border" />
            ) : (
          ...
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-foreground mb-1">{displayName}</h1>
            <div className="flex items-center gap-4 text-xs font-medium text-muted-foreground">
              <span className="flex items-center gap-1.5"><Star weight="fill" className="h-3.5 w-3.5 text-yellow-500" /> Top Rated Node</span>
              <span>•</span>
              <span className="flex items-center gap-1.5"><Pulse weight="fill" className="h-3.5 w-3.5 text-primary" /> 100% Success</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 bg-muted p-1.5 rounded-xl border border-border">
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
              role === "freelancer" ? "bg-muted/50 text-foreground shadow-md" : "text-muted-foreground/60 hover:text-foreground"
            }`}
          >
            Freelancer
          </button>
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${
              role === "recruiter" ? "bg-muted/50 text-foreground shadow-md" : "text-muted-foreground/60 hover:text-foreground"
            }`}
          >
            Recruiter
          </button>
          ...
          <Card className="relative w-full max-w-md bg-muted border-border shadow-2xl">
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <Input className="mt-1.5 bg-background border-border text-foreground" value={editName} onChange={(e) => setEditName(e.target.value)} />
              </div>
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Email Address</label>
                <Input className="mt-1.5 bg-background border-border text-foreground" type="email" value={editEmail} onChange={(e) => setEditEmail(e.target.value)} />
              </div>
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Avatar</label>
                <div className="mt-1.5 flex items-center gap-3 rounded-xl border border-border bg-background p-3">
                  ...
                  />
                  <Button size="sm" variant="secondary" className="bg-muted/50 text-foreground hover:bg-white/20 text-xs" onClick={() => profileAvatarInputRef.current?.click()}>
                    {uploadingAvatar ? "Uploading…" : "Change Avatar"}
                  </Button>
                </div>
              </div>
              <div className="flex justify-end gap-2 pt-4 border-t border-border">
                <Button variant="ghost" className="text-foreground hover:bg-muted/50" onClick={() => setProfileOpen(false)}>Cancel</Button>
      ...
      <div className="mb-10 grid grid-cols-2 gap-4 lg:grid-cols-4">
        {role === "freelancer" ? (
          <>
            ...
            <StatCard icon={TrendUp} label="Total Earnings" value={formatInrCompact(totalBidValue)} note="Lifetime value" />
          </>
        ) : (
          <>
            ...
            <StatCard icon={TrendUp} label="Total Spent" value={formatInrCompact(recruiterSpend)} note="Lifetime expenditure" />
          </>
        )}
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="flex items-center justify-between border-b border-border pb-4">
            <h2 className="text-xl font-bold text-foreground">{role === "freelancer" ? "Recommended For You" : "Your Postings"}</h2>
            {role === "recruiter" && (
              <Link to="/post-job">
                ...
                <Card className="bg-muted border-border hover:border-primary/30 transition-all cursor-pointer group shadow-md">
                  <CardContent className="p-6">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <Badge className={statusStyles[job.status] || "bg-muted/50"}>{job.status}</Badge>
                    </div>
                    <p className="text-sm text-foreground/60 line-clamp-2 mb-4">{job.description}</p>
                    <div className="flex flex-wrap gap-4 text-xs font-semibold text-muted-foreground/60">
                      ...
                      <span className="flex items-center gap-1.5 bg-background px-3 py-1.5 rounded-lg border border-border">
                        <TrendUp weight="bold" className="h-3.5 w-3.5 text-blue-400" /> ₹{formatUsdAsInr(job.budgetMax)} Budget
                      </span>
                      <span className="flex items-center gap-1.5 bg-background px-3 py-1.5 rounded-lg border border-border">
                        <Clock weight="fill" className="h-3.5 w-3.5 text-yellow-500" /> {job.deadline} deadline
                      </span>
                    </div>
                  </CardContent>
                </Card>
              ...
              <CardContent className="p-0">
                <div className="divide-y divide-white/5">
                  {myBids.slice(0, 3).map(bid => {
                    ...
                    return (
                      <Link key={bid.id} to="/jobs/$jobId" params={{ jobId: bid.jobId }} className="block p-4 hover:bg-muted transition-colors">
                        <p className="font-semibold text-sm text-foreground truncate">{job?.title || "Unknown Job"}</p>
                        <div className="flex items-center justify-between mt-2">
                          <span className="text-xs text-primary font-bold">₹{formatUsdAsInr(bid.amount)}</span>
                          <span className={`text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 rounded-full ${statusStyles[bid.status] || 'bg-muted/50'}`}>
                            {bid.status}
                          </span>
                        </div>
                      </Link>
                  ...
                  {myBids.length === 0 && (
                     <div className="p-6 text-center text-xs text-muted-foreground/60">You haven't submitted any proposals yet.</div>
                  )}
                </div>
              </CardContent>
            </Card>
